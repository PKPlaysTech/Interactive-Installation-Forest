...
function preload() {
  ...
  imgRose = loadImage('rose.png'); // 🌹 玫瑰图片
}
...
function handleInput() {
  if (handData && handData.length > 0) {
    ...
    if (smoothX < 260 && isPinching && !wasPinching) {
      ...
      else if (currentMode !== "flower" && currentMode !== "rose" && currentMode !== "butterfly" && currentMode !== "vine" && frameCount % 4 === 0) createNew(smoothX, smoothY, currentMode);
    }
...
function createNew(x, y, type) {
  ...
  if (type === "flower") {
    targetSize = random(40, 80); // 调小后的尺寸
    let h = random(360);
    colorMode(HSB, 360, 100, 100, 1);
    colors = [color(h, 70, 100, 0.7), color((h+40)%360, 60, 100, 0.8), color((h+80)%360, 40, 100, 0.9), color(0, 0, 100, 1)];
    colorMode(RGB, 255, 255, 255, 255);
  } else if (type === "roseFlower") {
    targetSize = random(70, 110);
  } else if (type === "roseStem") {
    targetSize = random(15, 25);
    colors = color(30, random(100, 180), 60, 200); // 玫瑰茎的深绿色
...
function drawFlower(el) {
  ...
  scale(breath); noStroke();
  for (let l = 0; l < 4; l++) {
    let c = el.colors[l];
    fill(c.levels[0], c.levels[1], c.levels[2], 200 * (1 - l*0.18));
...
function drawRose(el) {
  ...
  scale(1 + sin(frameCount * 0.06 + el.offset) * 0.05);
...
function drawMagicGrass(el) {
  ...
  fill(el.colors); noStroke(); beginShape(); vertex(-15, 0); vertex(15, 0);
  bezierVertex(10, -el.size*0.4, sw, -el.size*0.8, sw, -el.size); endShape(CLOSE); pop();
}
...
function drawButterfly(el) {
  ...
  scale(sin(frameCount * 0.2 + el.offset), 1); tint(255, 200, 255, 230);
...
function drawAndIdentifyFireflies() {
  ...
  for (let f of fireflies) {
    f.x += f.vx; f.y += f.vy; if(f.x<0 || f.x>width) f.vx*=-1; if(f.y<0 || f.y>height) f.vy*=-1;
