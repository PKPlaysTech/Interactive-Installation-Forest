...
function handleInput() {
  if (handData && handData.length > 0) {
    ...
    // 指尖引导点 (放在高 Z 轴，确保不被遮挡)
    push();
    translate(smoothX, smoothY, 200); 
    noStroke();
    if (isPinching) {
...
function drawUI() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    fill(isSelected ? 60 : 25, 220); 
    stroke(0, 255, 255, 120); 
    rect(i * btnW, 0, btnW, 85); 
    
    fill(255); 
    noStroke(); 
    textAlign(CENTER, CENTER); 
    textSize(24); text(icons[i], i * btnW + btnW/2, 35);
    textSize(11); text(labels[i], i * btnW + btnW/2, 65);
    
    if(isSelected) {
      fill(0, 255, 255);
      rect(i * btnW, 80, btnW, 5);
    }
  }
  pop();
}
...
function createNew(x, y, type) {
  ...
  if (type === "star") { finalX = x + random(-50, 50); finalY = y + random(-50, 50); targetSize = random(8, 22); col = color(255); }
  else if (type === "vine") { let g = [color(57, 255, 20), color(0, 255, 127), color(173, 255, 47)]; col = random(g); targetSize = random(20, 35); }
  else if (type === "flower") { let p = [color(255,0,127), color(191,0,255)]; col = random(p); targetSize = random(30, 50); }
  else if (type === "grass") { col = color(57, 255, 20); targetSize = random(40, 75); }
  else { col = color(255); targetSize = random(35, 50); }
  if (elements.length > 0) { let last = elements[elements.length - 1]; angle = atan2(y - last.y, x - last.x) + HALF_PI; }
...
function drawStar(el) { if (!imgStar) return; if (el.size < el.maxSize) el.size += 0.2; push(); translate(el.x - width/2, el.y - height/2, 1); imageMode(CENTER); blendMode(SCREEN); let b = sin(frameCount * el.blinkSpeed + el.offset); let a = map(b, -1, 1, 120, 255); tint(255, a); image(imgStar, 0, 0, el.size, el.size); blendMode(BLEND); pop(); }
